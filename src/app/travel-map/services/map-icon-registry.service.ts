...
export class MapIconRegistryService {
  ...
  private readonly registrySourcePaths: Record<ICON_TYPE, string> = {
  ...
  public getIcon(iconName: string, iconType: ICON_TYPE): Observable<string> {
    return (
      this.registries[iconType]?.pipe(
        ...
        catchError(() => of(null)),
        map((iconOrNull) => iconOrNull ?? this.unknownIcon),
      ) ?? of(this.unknownIcon)
    );
  }
  ...
  private parseIconSet(setString: string): Record<string, string> {
    const parseResult: Record<string, string> = {};
